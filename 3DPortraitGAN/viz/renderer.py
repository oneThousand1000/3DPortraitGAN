...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        if conditioning_body_pose is None:
            conditioning_body_pose = np.zeros((1, 6), dtype=np.float32)
        else:
            conditioning_body_pose = np.array(conditioning_body_pose, dtype=np.float32)
            conditioning_body_pose = np.reshape(conditioning_body_pose, (1, -1))
        ...
        all_ps = np.zeros([len(all_seeds), 6], dtype=np.float32)
        ...
        all_ps[idx, :] = conditioning_body_pose
        ...
        all_ps = self.to_device(torch.from_numpy(all_ps))


        all_ws = G.mapping(z=all_zs, c=all_cs, p=all_ps,truncation_psi=trunc_psi, truncation_cutoff=trunc_cutoff) - w_avg
        ...
        if not (body_pose == 0).all():
            apply_def = True
            body_pose = torch.tensor(body_pose).to(w.device)
        else:
            apply_def = False
            body_pose = None


        out, layers = self.run_synthesis_net(G, w, c, capture_layer=layer_name, apply_def=apply_def, pose_params=body_pose,
                                             **synthesis_kwargs)
        ...
        if cache_key not in self._net_layers:
            if layer_name is not None:
                torch.manual_seed(random_seed)
                _out, layers = self.run_synthesis_net(G, w, c, apply_def=apply_def, pose_params=body_pose,
                                                      **synthesis_kwargs)
