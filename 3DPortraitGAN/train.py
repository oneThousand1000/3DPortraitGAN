...
def init_dataset_kwargs(data,seg_data, data_rebalance,data_rebalance_idx_file,back_repeat):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.MaskLabeledDataset',
                                         img_path=data,
                                         seg_path = seg_data,
                                         back_repeat = back_repeat,
                                         use_labels=True, max_size=None, xflip=True,
                                         data_rebalance=data_rebalance,data_rebalance_idx_file=data_rebalance_idx_file)
...
@click.option('--resume_kimg',       help='Resume from given kimg', metavar='INT',  type=int, default=0)
@click.option('--freezed',      help='Freeze first layers of D', metavar='INT',                 type=click.IntRange(min=0), default=0, show_default=True)
@click.option('--data_rebalance',       help='Enable dataset rebalance', metavar='BOOL',                  type=bool, default=False, show_default=True)
@click.option('--data_rebalance_idx_file',       help='Enable dataset rebalance', metavar='BOOL',                  type=str, required=False,default = None)
...
@click.option('--image-snap',         help='How often to save snapshots', metavar='TICKS',            type=click.IntRange(min=1), default=50, show_default=True)
...
@click.option('--thickness',           help='sample thickness around head mesh', metavar='FLOAT',            type=click.FloatRange(min=0, max=1), default=0.25, show_default=True) # smpl head stride is ~ 0.2




@click.option('--pose_loss_weight',           help='sample thickness around head mesh', metavar='FLOAT',            type=click.FloatRange(min=0), default=1.0, show_default=True) # smpl head stride is ~ 0.2
@click.option('--input_pose_params_reg_loss_weight',           help='sample thickness around head mesh', metavar='FLOAT',            type=click.FloatRange(min=0), default=0, show_default=True) # smpl head stride is ~ 0.2
@click.option('--input_pose_params_reg_loss_kimg',           help='sample thickness around head mesh', metavar='INT',            type=click.FloatRange(min=0), default=0, show_default=True) # smpl head stride is ~ 0.2


@click.option('--explicitly_symmetry', help='Enable to blur even on resume', metavar='BOOL',  type=bool, required=False, default=False)


@click.option('--train_g_pose_branch', help='Enable to blur even on resume', metavar='BOOL',  type=bool, required=False, default=True)
...
@click.option('--seg_data',         help='Training data', metavar='[ZIP|DIR]',                      type=str, required=True)
...
def main(**kwargs):
    ...
    if opts.data_rebalance:
        assert opts.data_rebalance_idx_file is not None

    if opts.data_rebalance:
        raise NotImplementedError('data_rebalance is not implemented yet')

    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, seg_data =opts.seg_data, data_rebalance = opts.data_rebalance,data_rebalance_idx_file = opts.data_rebalance_idx_file,back_repeat = opts.back_repeat)
    if opts.cond and not c.training_set_kwargs.use_labels:
        raise click.ClickException('--cond=True requires labels specified in dataset.json')
    c.training_set_kwargs.use_labels = opts.cond
    c.training_set_kwargs.xflip = opts.mirror
    c.training_set_kwargs.data_rebalance = opts.data_rebalance
    if opts.data_rebalance:
        c.training_set_kwargs.data_rebalance_idx_file = opts.data_rebalance_idx_file
    ...
    c.G_kwargs.batch_size = c.batch_gpu
    ...
    c.G_kwargs.class_name = 'training.smpl_triplane.TriPlaneGenerator'
    c.D_kwargs.class_name = 'training.dual_discriminator.PoseShapeAwareDualDiscriminator'
    ...
    if opts.cfg == 'full-head':
        rendering_options.update({
            ...
            'ray_end': 3.3,  # far point along each ray to stop taking samples.
            'box_warp': 1,
            #'c_gen_conditioning_zero': True, # disable camera condition on mapping network
    ...
    c.G_kwargs.thickness = opts.thickness
    c.loss_kwargs.style_mixing_prob = opts.style_mixing_prob
    c.loss_kwargs.thickness = opts.thickness

    c.metric_pose_sample_mode = opts.metric_pose_sample_mode


    c.loss_kwargs.pose_loss_weight = opts.pose_loss_weight

    c.loss_kwargs.input_pose_params_reg_loss_weight = opts.input_pose_params_reg_loss_weight
    c.loss_kwargs.input_pose_params_reg_loss_kimg = opts.input_pose_params_reg_loss_kimg

    c.train_g_pose_branch = opts.train_g_pose_branch

    c.G_kwargs.explicitly_symmetry = opts.explicitly_symmetry
    c.D_kwargs.explicitly_symmetry = opts.explicitly_symmetry
    ...
    # Description string.
    desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma}'
