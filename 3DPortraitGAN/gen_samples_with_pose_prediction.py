...
@click.option('--pose_prediction_kwargs_path',  help='pose_prediction_kwargs filename', required=True)
...
def generate_images(
    ...
    mask_base_path = os.path.join(test_data, 'mask')
    ...
    with torch.no_grad():
        for real_image_path in real_image_paths:
            image_name = os.path.basename(real_image_path)
            mask_path = os.path.join(mask_base_path, image_name)
            print('projecting image: "%s"' % real_image_path)
