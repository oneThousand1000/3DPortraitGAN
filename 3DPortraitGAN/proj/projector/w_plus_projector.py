...
def project(
    ...
    if (not os.path.exists(w_avg_path)) or (not os.path.exists(w_std_path)):
        ...
        w_samples = G.mapping(torch.from_numpy(z_samples).to(device), c_samples, p_samples, truncation_psi=1.0,
                              truncation_cutoff=None )  # [N, L, C]
    ...
    for step in tqdm(range(num_steps)):
        ...
        synth_images = G.synthesis(ws, c=c, noise_mode='const', apply_def=True, pose_params=p)['image']
        if step % image_log_step == 0:
            with torch.no_grad():
                vis_img = (synth_images.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)

                PIL.Image.fromarray(vis_img[0].cpu().numpy(), 'RGB').save(f'{outdir}/{step}_w_plus.png')
